...
const SYSTEM_PROMPT = `
You are an assistence that receives a list of ingredients that a user has and suggest a recipe they
...
const hfApiKey = new InferenceClient(import.meta.env.VITE_HF_ACCESS_TOKEN);

export async function getRecipeFromMistral(ingredientsArr) {
  const ingredientsString = ingredientsArr.join(", ");
  try {
    const response = await hfApiKey.chatCompletion({
